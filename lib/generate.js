(function(root, factory) {
    ...
    function serializeRule(rule, builtinPostprocessors) {
        ...
        if (rule.updateContext) {
            ret += ', "updateContext": ' + tabulateString(dedentFunc(rule.updateContext), '        ', {indentFirst: false});
        } else {
            ret += ', "updateContext": (generator, context) => context';
        }
        ret += '}';
        return ret;
    }
...
export interface Token {
    produce: (generator, context: any) => any;
    name?: string;
...
export type NearleySymbol = Token | Literal | string
...
export function compileGrammar(valueGenerator): Compiled {
    ...
    const ParserRules: NearleyRule[] = ${serializeRules(parser.rules, generate.jstestfuzz.builtinPostprocessors)};
