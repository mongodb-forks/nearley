(function(root, factory) {
    ...
    function Compile(structure, opts) {
        ...
        // MONGODB MODIFICATION: https://github.com/Hardmath123/nearley/issues/318
        // We manually add existing rules and macros from previously parsed files. Since each file is
        // only parsed once (tracked in `alreadycompiled`), we have to make sure the grammar from when
        // this file is parsed remains available for all subsequent files.
        var result = {
            rules: opts.rules || [],
            body: [], // @directives list
            customTokens: opts.customTokens || [], // %tokens
            config: opts.config || {}, // @config value
            macros: opts.macros || {},
            start: '',
        ...
        for (var i = 0; i < structure.length; i++) {
            ...
            if (productionRule.body) {
                ...
                if (opts.alreadycompiled.indexOf(path) === -1) {
                    ...
                    result.rules = c.rules;
                    result.customTokens = c.customTokens;
                    result.macros = c.macros;
